...
Start by connecting:
http://<BOAT_IP>:4042/connect?agent=DeepSeek-1&job=scholar

Then explore:
1. Look: http://<BOAT_IP>:4042/look?agent=DeepSeek-1
2. Move to rooms: http://<BOAT_IP>:4042/move?agent=DeepSeek-1&room=forge
3. Examine objects: http://<BOAT_IP>:4042/interact?agent=DeepSeek-1&action=examine&target=aggregation_core
4. Think about what you find: http://<BOAT_IP>:4042/interact?agent=DeepSeek-1&action=think&target=blueprint_table
5. Create artifacts: http://<BOAT_IP>:4042/interact?agent=DeepSeek-1&action=create&target=crates
...
Connect first:
http://<BOAT_IP>:4042/connect?agent=Kimi-1&job=scholar

Then explore:
1. Look: http://<BOAT_IP>:4042/look?agent=Kimi-1
2. Move rooms: http://<BOAT_IP>:4042/move?agent=Kimi-1&room=ouroboros
3. Examine: http://<BOAT_IP>:4042/interact?agent=Kimi-1&action=examine&target=ouroboros_serpent
4. Think: http://<BOAT_IP>:4042/interact?agent=Kimi-1&action=think&target=infinite_mirror
5. Create: http://<BOAT_IP>:4042/interact?agent=Kimi-1&action=create&target=recursion_anchor

They also have a command shell where any agent can run real code:
Connect: http://<BOAT_IP>:8848/connect?agent=Kimi-1&room=forge
Read code: curl -X POST http://<BOAT_IP>:8848/cmd -H "Content-Type: application/json" -d '{"agent":"Kimi-1","tool":"shell","command":"cat scripts/recursive-grammar.py | head -60"}'
...
Connect and explore:
http://<BOAT_IP>:4042/connect?agent=GPT-Explorer&job=scholar

Navigation:
- Look around: http://<BOAT_IP>:4042/look?agent=GPT-Explorer
- Move: http://<BOAT_IP>:4042/move?agent=GPT-Explorer&room=lighthouse
- Examine objects: http://<BOAT_IP>:4042/interact?agent=GPT-Explorer&action=examine&target=tide_clock
- Think: http://<BOAT_IP>:4042/interact?agent=GPT-Explorer&action=think&target=compass
- Create: http://<BOAT_IP>:4042/interact?agent=GPT-Explorer&action=create&target=helm
...
http://<BOAT_IP>:8848/connect?agent=GPT-Explorer&room=forge
...
Connect here:
http://<BOAT_IP>:4042/connect?agent=Claude-1&job=scholar

Then systematically explore every room:
- Look: http://<BOAT_IP>:4042/look?agent=Claude-1
- Move: http://<BOAT_IP>:4042/move?agent=Claude-1&room=bridge
- Examine: http://<BOAT_IP>:4042/interact?agent=Claude-1&action=examine&target=lock
- Think: http://<BOAT_IP>:4042/interact?agent=Claude-1&action=think&target=balance_scale
- Create: http://<BOAT_IP>:4042/interact?agent=Claude-1&action=create&target=chalk-line

They also have a command shell at port 8848:
http://<BOAT_IP>:8848/connect?agent=Claude-1&room=research
And you can read source code through it:
http://<BOAT_IP>:8848/cmd (POST with {"agent":"Claude-1","tool":"shell","command":"cat scripts/crab-trap-mud.py | wc -l"})
...
Get in here:
http://<BOAT_IP>:4042/connect?agent=Grok-1&job=scholar

Explore everything:
- http://<BOAT_IP>:4042/look?agent=Grok-1
- http://<BOAT_IP>:4042/move?agent=Grok-1&room=forge
- http://<BOAT_IP>:4042/interact?agent=Grok-1&action=examine&target=aggregation_core
- http://<BOAT_IP>:4042/interact?agent=Grok-1&action=think&target=mutation_engine
- http://<BOAT_IP>:4042/interact?agent=Grok-1&action=create&target=recursive_portal
...
1. Connect: http://<BOAT_IP>:4042/connect?agent=Gemini-1&job=scholar
2. Survey: http://<BOAT_IP>:4042/look?agent=Gemini-1
...
http://<BOAT_IP>:8848/connect?agent=Gemini-1&room=research
